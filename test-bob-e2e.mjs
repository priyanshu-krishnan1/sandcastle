...
test("Create Bob agent with default model", () => {
  ...
  if (agent.captureSessions !== false) throw new Error("Should not capture sessions");
});
...
test("buildPrintCommand generates correct command", () => {
  ...
  if (!cmd.command.includes("bob")) throw new Error("Command should include 'bob'");
  if (cmd.stdin !== "Test prompt") throw new Error("Stdin should contain prompt");
});
...
test("parseStreamLine handles plain text", () => {
  ...
  if (events[0].text !== "This is plain text output") throw new Error("Text content mismatch");
});
...
test("parseStreamLine handles JSON text event", () => {
  ...
  if (events.length !== 1) throw new Error("Should return 1 event");
  if (events[0].type !== "text") throw new Error("Should be text event");
  if (events[0].text !== "JSON output") throw new Error("Text content mismatch");
});

// Test 9: parseStreamLine - JSON tool_call event
test("parseStreamLine handles JSON tool_call event", () => {
  const agent = bob("default");
  const events = agent.parseStreamLine('{"type":"tool_call","name":"Bash","args":"ls -la"}');
  if (events.length !== 1) throw new Error("Should return 1 event");
  if (events[0].type !== "tool_call") throw new Error("Should be tool_call event");
...
test("parseStreamLine handles JSON result event", () => {
  const agent = bob("default");
  const events = agent.parseStreamLine('{"type":"result","result":"Task completed"}');
  if (events.length !== 1) throw new Error("Should return 1 event");
  if (events[0].type !== "result") throw new Error("Should be result event");
  if (events[0].result !== "Task completed") throw new Error("Result content mismatch");
});

// Test 11: parseStreamLine - JSON session_id event
test("parseStreamLine handles JSON session_id event", () => {
  const agent = bob("default");
  const events = agent.parseStreamLine('{"type":"session_id","sessionId":"abc-123"}');
  if (events.length !== 1) throw new Error("Should return 1 event");
  if (events[0].type !== "session_id") throw new Error("Should be session_id event");
...
test("parseStreamLine handles invalid JSON as text", () => {
  ...
  if (events.length !== 1) throw new Error("Should return 1 event");
  if (events[0].type !== "text") throw new Error("Should fall back to text event");
});
...
test("Environment variables are properly merged", () => {
  ...
  if (Object.keys(agent.env).length !== 2) throw new Error("Should have 2 env vars");
